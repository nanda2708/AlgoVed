...
const publicResults = (results = []) => results.map(({ input, expected, actual, passed, hidden, status }) => (
...
export const createContestSubmission = async (req, res) => {
  ...
  try {
    if (!mongoose.isValidObjectId(problemId) || !mongoose.isValidObjectId(contestId)) {
      return res.status(400).json({ message: 'Invalid problem or contest ID' });
    }
    ...
    const now = Date.now();
    if (now < new Date(contest.startTime).getTime() || now > new Date(contest.endTime).getTime()) {
      return res.status(400).json({ message: 'Contest is not active' });
    }
    if (!contest.participants.some((id) => String(id) === String(req.user.userId))) {
      return res.status(403).json({ message: 'Join the contest before submitting' });
    }
    ...
    res.status(201).json({
      ...
      language,
      status,
      testCaseResults: publicResults(results),
...
export const getContestSubmissions = async (req, res) => {
  ...
  try {
    ...
    res.json(submissions.map((submission) => ({ ...submission, testCaseResults: publicResults(submission.testCaseResults) })));
  } catch (err) {
