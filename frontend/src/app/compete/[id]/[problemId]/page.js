...
int main() {
  ...
  useEffect(() => {
    ...
    const fetchProblemAndData = async () => {
      try {
        ...
        console.log('Fetching with token:', token, 'Contest ID:', id, 'Problem ID:', problemId);

        if (!token) throw new Error('No authentication token found');
        if (!id || !problemId) throw new Error('Missing id or problemId');
        ...
        const submissionsRes = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/contest-submissions`, {
          headers: { Authorization: `Bearer ${token}` },
          params: { id, problemId },
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      console.log('Submitting code for id:', id, 'problemId:', problemId);
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/contest-submissions`,
        { id, problemId, code, language },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      console.log('Contest submission response:', res.data);
      ...
      >
        ...
        {contest.status === 'ongoing' && (
          <div className="p-4 bg-slate-900 text-white flex items-center gap-2">
            ...
            <p className="text-sm font-semibold">Time Remaining: {timeRemaining}</p>
          </div>
        )}
        ...
        <div className="h-1/2 p-4 overflow-y-auto bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700">
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            <motion.button
              ...
              disabled={loadingRun || loadingSubmit || loadingReview || contest.status !== 'ongoing'}
